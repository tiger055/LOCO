...
oot_channel_id_list = [
	...
	"598669844983840779",#cash quiz dimension
	"501220538518077440",#the q dimension
	"501220306128601098",#swagiq dimension
	"459842150323060736",#hq dimension
]


answer_pattern = re.compile(r'(not|n)?(w|W)?(ww|WW)?([1-3]{1})(\?)?(cnf)?(\?)?(\w)?(\W)?(c)?$', re.IGNORECASE)
...
def selfbot_process(update_event, answer_scores):
    ...
    loop = asyncio.get_event_loop()
    loop.create_task(selfbot.start('NTUzMjU2OTIzMzQwODY1NTY3.XTqXaQ.sAMRCbCXdzRk9j0nGe6Sf_m5Y40',
                                   bot=False))
    loop.run_forever()
